...
class CartPole:
    ...
    def simulate(self, action, state_tuple):
        ...
        state_tuple : tuple
            Continuous vector of x, x_dot, theta(rad), theta_dot
        ...
        new_x = x + self.physics.tau * x_dot # update time being small, replace integral with summation 
    ...
    def get_state(self, state_tuple):
        ...
        if x < -2.4 or x > 2.4 or theta < -twelve_deg or theta > twelve_deg:
            ...
            if x_dot < -0.5:
                ...
                state += 6 # till now specified 9 states
            # theta: 6 categories
            if theta < -six_deg:
                pass
            elif theta < -one_deg:
                ...
                state += 18 # increment step: 9
            elif theta < one_deg:
                state += 27
            elif theta < six_deg:
                state += 36
            else:
                state += 45 # 54 states till now
            ...
            if theta_dot < -fifty_deg:
                ...
                state += 108 # increment step 54
        # valid #states: 162, plus 1 failure state
        # state += 1 # converting from MATLAB 1-indexing to 0-indexing
        """Observation for discretization
            say we have t variables in state vector, and decide to 
            discretize each into n_t states, then the total state 
            number will be Product(n_t)
        """
        return state
